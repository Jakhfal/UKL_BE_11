...
const createBorrow = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const findItem = await prisma.inventory.findFirst({where : {id : item_id}})
        if (!findItem) {
            res.status(400).json({
...
const analyzeUsage = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const borrowData = await prisma.borrow.findMany({
            ...
            include: {
                inventory_detail: true, // Include relation ke tabel Items
            },
        });
        ...
        const groupedData = borrowData.reduce((acc: Record<string, any>, borrow) => {
            const group = borrow.inventory_detail[group_by as keyof typeof borrow.inventory_detail] as string; // Type assertion
            if (!acc[group]) {
                acc[group] = {
                    group,
                    total_borrowed: 0,
...
const analyzeItemUsage = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        const borrowData = await prisma.borrow.findMany({
            ...
            include: {
                inventory_detail: true, // Sertakan detail item
            },
        });
        ...
        borrowData.forEach((borrow) => {
            ...
            if (!frequentlyBorrowedItems[itemId]) {
                frequentlyBorrowedItems[itemId] = {
                    item_id: itemId,
                    name: borrow.inventory_detail.name,
                    category: borrow.inventory_detail.category,
                    total_borrowed: 0,
                };
            }
        ...
        borrowData.forEach((borrow) => {
            ...
            if (!inefficientItems[item_id]) {
                inefficientItems[item_id] = {
                    item_id: item_id,
                    name: borrow.inventory_detail.name,
                    category: borrow.inventory_detail.category,
                    total_borrowed: 0,
            ...
            inefficientItems[item_id].total_borrowed++;
            ...
            if (returnRecord && new Date(returnRecord.actual_return_date) > new Date(borrow.return_date)) {
                inefficientItems[item_id].total_late_returns++;
            }
        });
